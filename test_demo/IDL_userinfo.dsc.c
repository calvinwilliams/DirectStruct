...
int DSCSERIALIZE_JSON_userinfo( userinfo *pst , char *encoding , char *buf , int *p_len )
{
	...
	len=SNPRINTF(buf,remain_len,"	{\n"); if( len <= 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"		\"user_id\" : "); if( len < 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"%d",pst->user_id); JSONESCAPE_EXPAND(buf,len,remain_len); if( len < 0 ) return -1;
	len=SNPRINTF(buf,remain_len," ,\n"); if( len < 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"		\"user_name\" : "); if( len < 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"\""); if( len < 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"%s",pst->user_name); JSONESCAPE_EXPAND(buf,len,remain_len); if( len < 0 ) return -1;
	len=SNPRINTF(buf,remain_len,"\""); if( len < 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len," ,\n"); if( len < 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"		\"email\" : "); if( len < 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"\""); if( len < 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"%s",pst->email); JSONESCAPE_EXPAND(buf,len,remain_len); if( len < 0 ) return -1;
	len=SNPRINTF(buf,remain_len,"\""); if( len < 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"\n"); if( len < 0 ) return -1; buf+=len; remain_len-=len;
			len=SNPRINTF(buf,remain_len,"	}\n"); if( len <= 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"}\n"); if( len <= 0 ) return -1; buf+=len; remain_len-=len;
...
int CallbackOnJsonNode_userinfo( int type , char *jpath , int jpath_len , int jpath_size , char *nodename , int nodename_len , char *content , int content_len , void *p )
{
	userinfo	*pst = (userinfo*)p ;
	int	index[10] = { 0 } ; index[0] = 0 ;
	...
	{
		/* user_id */
		if( strncmp( jpath , "/userinfo/user_id" , jpath_len ) == 0 )
		{NATOI(content,content_len,pst->user_id);}
		/* user_name */
		if( strncmp( jpath , "/userinfo/user_name" , jpath_len ) == 0 )
		{JSONUNESCAPE_FOLD(content,content_len,pst->user_name);
		{if( content_len > sizeof(pst->user_name)-1 ) return -7;}}
		/* email */
		if( strncmp( jpath , "/userinfo/email" , jpath_len ) == 0 )
		{JSONUNESCAPE_FOLD(content,content_len,pst->email);
		{if( content_len > sizeof(pst->email)-1 ) return -7;}}
	}
