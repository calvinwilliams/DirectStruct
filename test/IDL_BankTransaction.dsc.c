...
int DSCNETORDER_BankTransaction( BankTransaction *pst )
{
	int	index[10] = { 0 } ; index[0] = 0 ;
		...
		pst->ResponseHeader.response_code=(int)htonl((unsigned int)(pst->ResponseHeader.response_code));
			...
			for( index[2] = 0 ; index[2] < 10 ; index[2]++ )
			{
			__HTONF(pst->QueryTransactionDetails.TransactionDetails[index[2]].trans_amount);
			}
	return 0;
}
...
int DSCHOSTORDER_BankTransaction( BankTransaction *pst )
{
	int	index[10] = { 0 } ; index[0] = 0 ;
		...
		pst->ResponseHeader.response_code=(int)ntohl((unsigned int)(pst->ResponseHeader.response_code));
			...
			for( index[2] = 0 ; index[2] < 10 ; index[2]++ )
			{
			__NTOHF(pst->QueryTransactionDetails.TransactionDetails[index[2]].trans_amount);
			}
	return 0;
}
...
int DSCSERIALIZE_COMPACT_BankTransaction( BankTransaction *pst , char *buf , int *p_len )
{
	...
	len++; ptr++;
		...
		len+=1024; ptr+=1024;
			...
			len+=20; ptr+=20;
			/* trans_amount */
			{ float f=pst->QueryTransactionDetails.TransactionDetails[index[2]].trans_amount; __HTONF(f); memcpy( ptr , (char*)&f , 4 ); }
			len+=4; ptr+=4;
			}
...
int DSCDESERIALIZE_COMPACT_BankTransaction( char *buf , int *p_len , BankTransaction *pst )
{
	...
	len++; ptr++;
		...
		len+=1024; ptr+=1024;
			...
			len+=20; ptr+=20;
			/* trans_amount */
			memcpy( (char*)&(pst->QueryTransactionDetails.TransactionDetails[index[2]].trans_amount) , ptr , 4 ); __NTOHF(pst->QueryTransactionDetails.TransactionDetails[index[2]].trans_amount);
			len+=4; ptr+=4;
			}
