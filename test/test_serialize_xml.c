...
int test_serialize_xml()
{
	...
	memset( buf , 0x00 , sizeof(buf) );
	len = sizeof(buf)-1 ;
	nret = DSCSERIALIZE_XML_COMPACT_BankXmlTransaction( & trans , "GBK" , buf , & len ) ;
	if( nret )
	{
		printf( "DSCSERIALIZE_XML_COMPACT_BankXmlTransaction failed[%d]\n" , nret );
		return nret;
	}
	else
	{
		printf( "DSCSERIALIZE_XML_COMPACT_BankXmlTransaction ok , len[%d]\n" , len );
	}
